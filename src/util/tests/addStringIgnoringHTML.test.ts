...
describe("Normal Cases", () => {
	test("Inserts Into A Normal String", () => {
		...
		const stringWithInsertion = addStringIgnoringHTML(basisString, stringToAdd, 7);
	...
	test("Ignores A Single HTML Tag", () => {
		...
		const stringWithInsertion = addStringIgnoringHTML(basisString, stringToAdd, 8);
	...
	test("Doubles Up Strings If Specified", () => {
		const basisString: string = "This is a test";
		const stringToAdd: string = " a";
		const stringWithInsertion = addStringIgnoringHTML(basisString, stringToAdd, 7, true);
...
describe("Limit Cases", () => {
	test("Will Add To The Beginning Of the String With Position = 0", () => {
		...
		const stringWithInsertion = addStringIgnoringHTML(basisString, stringToAdd, 0);
		expect(stringWithInsertion).toStrictEqual("This is a test");
	});
	...
	test("Will Add To The End Of the String With Position = basisString.length + 1", () => {
		...
		const stringWithInsertion = addStringIgnoringHTML(basisString, stringToAdd, basisString.length + 1);
		expect(stringWithInsertion).toStrictEqual("This is a test");
	});
